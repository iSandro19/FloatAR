...
public class IACreateBoard {
    ...
    /**
     * Rellenar el tablero con barcos
     */
    ...
    /**
     * Comprobar si se puede colocar un barco en la posición indicada
     * @param boatType Tipo de barco
     * @param col Columna
     * @param row Fila
    ...
    private boolean isValidPosition(int boatType, int col, int row, int orientation){   // 1 = Vertical, 0 = Horizontal
        ...
        if (orientation == 0) {
            ...
            for (int i = 0; i<=boatType; i++) {
                ...
                if (actualPos <= max) {
                    if (this.board[actualPos][row] == 0) {
                        ...
                        if(izq < min) {
                            if (this.board[actualPos][der] == 1)
                                return false;
                        } else {
                            if (der > max) {
                                if(this.board[actualPos][izq] == 1)
                                    return false;
                            }
                            else if (this.board[actualPos][der] == 1 || this.board[actualPos][izq] == 1)
                                return false;
                        }
                    } else {
                        return false;
            ...
            if ((col + boatType + 1) < max) {
                return this.board[col + boatType + 1][row] != 1;
            }
        // Lo mismo, pero para barcos verticales
            ...
            for (int i = 0; i<=boatType; i++) {
                ...
                if (actualPos <= max) {
                    if (this.board[col][actualPos] == 0) {
                        // Hacemos estas comprobaciones de izq y der para evitar salirse de los límites y que de un NullPointerException
                        if (izq < min) {
                            if (this.board[der][actualPos] == 1)
                                return false;
                        } else {
                            if (der > max) {
                                if (this.board[izq][actualPos] == 1)
                                    return false;
                            }
                            else if (this.board[der][actualPos] == 1 || this.board[izq][actualPos] == 1)
                                return false;
                        }
                    } else {
                        return false;
            ...
            if ((row + boatType + 1) < max) {
                return this.board[col][row + boatType + 1] != 1;
            }
        }
